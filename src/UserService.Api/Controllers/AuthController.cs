...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequest dto, CancellationToken cancellationToken)
    {
        ...
        // --- 400: Invalid password format (must be encrypted) ---
        ...
        {
            _logger.LogWarning(ex, "Password decryption failed during login for email: {Email}", MaskEmail(dto.Email));
            return BadRequest(ErrorResponse("invalid_password_format", "The password format is invalid."));
        }
    ...
    public async Task<IActionResult> SocialLoginCallback([FromBody] SocialLoginRequest request, CancellationToken cancellationToken)
    {
        // --- 400: Null body ---
        if (request == null)
    ...
    public async Task<IActionResult> LinkSocialAccount([FromBody] LinkSocialAccountRequest request, CancellationToken cancellationToken)
    {
        // --- 400: Null body ---
        if (request == null)
    ...
    public async Task<IActionResult> UnlinkSocialAccount(string provider, CancellationToken cancellationToken)
    {
        // --- 400: Empty provider ---
    ...
    public async Task<IActionResult> GetLinkedAccounts(CancellationToken cancellationToken)
    {
        try
        {
            ...
            return Ok(accounts ?? new List<LinkedSocialAccountDto>());
        }
        // --- 429: Rate limited ---
        catch (RateLimitExceededException ex)
        {
